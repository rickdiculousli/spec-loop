...
- Zone workers are dispatched with `run_in_background: false` (per §4 step 1) — deliberately, not incidentally: `SendMessage` resumes an agent from its transcript, and a background dispatch is the one variable in this loop's control that's known to put that transcript at risk. Don't background a zone worker to parallelize scheduling; that trade isn't worth losing resumability.
...
1. Record the branch's current `HEAD` as this task's BASE — needed by step 3's diff capture; never substitute `HEAD~1` later, since a task's fix-up commits (step 5) can put more than one commit between BASE and the review, and `HEAD~1` would silently drop the earlier ones. Then dispatch the impl worker (Agent tool: `model: sonnet`, `run_in_background: false`). Zone follow-ups go via SendMessage and state the tree delta since the worker's last turn (`git status --porcelain` + diff — this also catches validation side-effects like regenerated artifacts). Empty delta → say "tree unchanged since your last turn"; non-empty → list the files, re-read required only where they intersect its task. **If the SendMessage resume itself fails** (agent not found, transcript not found, or any other resume error) — do not retry the same ID: the zone worker is lost, forced-retirement (§7) applies immediately, no retention-flex. Dispatch a fresh worker for the task, brief it as usual, and seed it with the zone's last persisted handoff note from the scratchpad ledger (§3) — that note, not the dead thread, is the recovery path.
...
**Forced retirement:** a resume failure (SendMessage errors — agent lost, transcript not found; seen in practice across machines/providers, e.g. Bedrock) retires the worker immediately, bypassing the soft thresholds above. No final turn is possible — you can't ask a dead thread to distill anything — so the successor is seeded only from whatever handoff note is already on disk in the ledger (§3, §4 step 1). This is why the ledger is written after *every* task, not just at refresh points: it's the only recovery path when a thread disappears without warning.
