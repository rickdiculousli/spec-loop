...
- Config knobs are env vars read at runtime, set via `settings.json` `"env"`: `LEGIBLE_BASH` (hook mode), `SPEC_LOOP_PUSH` (`auto` default / `off` — gates the pushes in `spec.sh save`/`start`), and `SPEC_LOOP_SPECS` (`git` default / `local` — gates whether `spec.sh new/save/start/done` touch git under `specs/`, the whole directory). All three are documented in README's Configuration section; keep that table in sync when adding a knob.
- Under `SPEC_LOOP_SPECS=local`, the entire `specs/` directory is git-ignored via one blanket `specs/.gitignore` (not per-slug) — `spec.sh new/save/start/done` never commit anything under it, including root-level files like `specs/README.md`/`specs/HOUSE-RULES.md`, so none of it lands in git on its own; only `/implement`'s code commits do. `.gitignore` never un-tracks an already-committed path, so this is safe alongside specs that are already tracked. `spec.sh untrack`/`track` flip things across that boundary, one commit each — `<slug>` for a single spec, `--all` to sweep everything under `specs/` at once on the current branch; `track` is the explicit, deliberate way to put local content into git.
...
`bash tests/run.sh` runs every `tests/test-*.sh`. Current coverage: the hook's block/warn/off/fail-open behavior and quote-stripping (`test-legible-bash.sh`), the `SPEC_LOOP_PUSH` knob (`test-push-knob.sh`, which builds a throwaway repo + bare origin in `mktemp -d`), the `brief`/`diff` subagent-handoff generation (`test-dispatch.sh`), the Success-criteria coverage-gate warning (`test-coverage-gate.sh`, a throwaway repo via `mktemp -d`, no remote needed since `check` never pushes), and the `SPEC_LOOP_SPECS` knob plus `untrack`/`track` (`test-local-specs.sh`, three throwaway repos in `mktemp -d`: one exercising local mode through `new/save/start/done`, one exercising `untrack <slug>`/`track <slug>` on a single already-committed spec, one exercising `untrack --all`/`track --all` across a repo with multiple already-landed specs plus a `specs/README.md`). Extend these when touching the scripts — e.g. new hook rules get a `check` line, new `spec.sh` behavior gets assertions in the sandbox repo.
