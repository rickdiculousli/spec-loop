...
- **Branch name = folder name = `<slug>`.** The spec is the branch's first commit. Every later edit — including spec deviations discovered mid-implementation (revised tasks, scope corrections) — lands on the branch. **The default branch is never written directly**; it receives a spec and its implementation together, when the branch merges (squash merge recommended: per-task commits are loop machinery, not history). Exception: under `SPEC_LOOP_SPECS=local`, none of the git behavior above applies — `spec.sh` never commits anything under `specs/` at any point (not this spec, not any other, not `specs/README.md`/`specs/HOUSE-RULES.md`), so none of it becomes part of the branch's git history, and the default branch receives only `/implement`'s code commits when a branch merges. `spec.sh untrack --all` / `track --all` flip the whole directory across that boundary at once; `untrack <slug>` / `track <slug>` do the same for one spec.
