...
- **shared-run-constants** (`specs/shared-run-constants/proposal.md`) — a `spec.sh env` subcommand plus a `Resolved constants:` brief-template line, sharing run-static values (repo root, default branch, plugin paths) across orchestrator/workers/verifiers to cut resolve-then-paste round-trips under the legible-bash doctrine. Deferred, not rejected: the pain is hypothetical — no observed run waste yet, and orchestrator-pastes-literals already covers most of it. Re-entry: an `/implement` run transcript showing ≥5 round-trips spent on run-static values, or two subagents in one run re-resolving the same constant.
