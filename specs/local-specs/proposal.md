...
- `/spec-setup`'s mandatory configuration-knobs question grows a third knob alongside `LEGIBLE_BASH`/`SPEC_LOOP_PUSH`.
...
- Current: `/spec-setup`'s configuration-knobs question covers only `LEGIBLE_BASH` and `SPEC_LOOP_PUSH`. Target: it also surfaces `SPEC_LOOP_SPECS` with the same keep-or-customize flow. Acceptance: `skills/spec-setup/SKILL.md`'s Configuration knobs section documents three knobs.
