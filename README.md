...
The `-C` check (git, go, make) compares the flag's argument against the hook's `cwd` input
(only available when the harness supplies it); a `-C` pointed anywhere other than cwd, or
